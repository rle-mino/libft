...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2016/02/04 18:34:35 by rle-mino          #+#    #+#             */
/*   Updated: 2016/02/08 06:13:55 by rle-mino         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "ft_printf.h"

static int			deci_prec_hh(t_struct data, char nbr, int i)
{
	int		k;
...
static int			deci_min_hh(t_struct data, char nbr, int i)
{
	int		j;
	int		k;
...
static int			deci_hash_hh(t_struct data, char nbr, int i)
{
	int		j;
	int		k;
...
int					deci_conv_hh(t_struct data, char nbr)
{
	...
	k = 0;
	i = pf_retdeci_hh(nbr, 2);
	if (data.prec == -1 && nbr == 0)
		i = 0;
	data.min = data.flag & 1 ? data.min - 2 : data.min;
	if (data.flag & 4 && data.min != 0)
		k = deci_hash_hh(data, nbr, i);
	else if (data.min != 0)
		k = deci_min_hh(data, nbr, i);
	else
		k = deci_prec_hh(data, nbr, i);
	return (k + i);
}
