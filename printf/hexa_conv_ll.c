...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2016/01/29 16:51:29 by rle-mino          #+#    #+#             */
/*   Updated: 2016/02/08 06:41:56 by rle-mino         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "ft_printf.h"

int		hexa_prec_ll(t_struct data, unsigned long long nbr, int i, int lower)
{
	int		k;
	...
	pf_puthexa_noflag_ll(nbr, lower);
	return (k);
}

int		hexa_min_ll(t_struct data, unsigned long long nbr, int i, int lower)
{
	int		j;
	int		k;
	int		l;
...
int		hexa_hash_ll(t_struct data, unsigned long long nbr, int i, int lower)
{
	int		j;
	int		k;
...
int		hexa_conv_ll(t_struct data, unsigned long long nbr, int lower)
{
	...
	k = 0;
	i = pf_rethexa_ll(nbr, 2);
	if (nbr == 0 && data.min != 0)
		i = 0;
	data.min = data.flag & 0b00000001 ? data.min - 2 : data.min;
	if (data.flag & 0b00000100 && data.min != 0)
		k = hexa_hash_ll(data, nbr, i, lower);
	else if (data.min != 0)
		k = hexa_min_ll(data, nbr, i, lower);
	else
		k = hexa_prec_ll(data, nbr, i, lower);
	if (data.prec != -1)
		return (k + i);
	return (k);
}
