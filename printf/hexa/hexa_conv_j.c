...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2016/01/29 16:51:35 by rle-mino          #+#    #+#             */
/*   Updated: 2016/02/08 06:41:26 by rle-mino         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "ft_printf.h"

static int		hexa_prec_j(t_struct data, uintmax_t nbr, int i, int lower)
{
	int		k;
...
static int		hexa_min_j(t_struct data, uintmax_t nbr, int i, int lower)
{
	int		j;
	int		k;
	int		l;
...
static int		hexa_hash_j(t_struct data, uintmax_t nbr, int i, int lower)
{
	int		j;
	int		k;
...
int				hexa_conv_j(t_struct data, uintmax_t nbr, int lower)
{
	...
	k = 0;
	i = pf_rethexa_j(nbr, 2);
	if (nbr == 0 && data.min != 0)
		i = 0;
	data.min = data.flag & 0b00000001 ? data.min - 2 : data.min;
	if (data.flag & 0b00000100 && data.min != 0)
		k = hexa_hash_j(data, nbr, i, lower);
	else if (data.min != 0)
		k = hexa_min_j(data, nbr, i, lower);
	else
		k = hexa_prec_j(data, nbr, i, lower);
	if (data.prec != -1)
		return (k + i);
	return (k);
}
