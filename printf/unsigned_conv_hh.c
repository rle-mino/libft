...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2016/02/05 10:53:34 by rle-mino          #+#    #+#             */
/*   Updated: 2016/02/08 06:46:46 by rle-mino         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "ft_printf.h"

static int			unsigned_prec_hh(t_struct data, unsigned char nbr, int i)
{
	int		k;
...
int					unsigned_conv_hh(t_struct data, unsigned char nbr)
{
	...
	k = 0;
	i = pf_retunsigned_hh(nbr, 2);
	if (data.prec == -1 && nbr == 0)
		i = 0;
	data.min = data.flag & 1 ? data.min - 2 : data.min;
	if (data.flag & 4 && data.min != 0)
		k = unsigned_hash_hh(data, nbr, i);
	else if (data.min != 0)
		k = unsigned_min_hh(data, nbr, i);
	else
		k = unsigned_prec_hh(data, nbr, i);
	return (k + i);
}
