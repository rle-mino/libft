...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2016/02/04 18:34:20 by rle-mino          #+#    #+#             */
/*   Updated: 2016/02/08 06:13:38 by rle-mino         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "ft_printf.h"

static int			deci_prec_h(t_struct data, short nbr, int i)
{
	int		k;
...
static int			deci_min_h(t_struct data, short nbr, int i)
{
	int		j;
	int		k;
...
static int			deci_hash_h(t_struct data, short nbr, int i)
{
	int		j;
	int		k;
...
int					deci_conv_h(t_struct data, short nbr)
{
	...
	k = 0;
	i = pf_retdeci_h(nbr, 2);
	if (data.prec == -1 && nbr == 0)
		i = 0;
	data.min = data.flag & 1 ? data.min - 2 : data.min;
	if (data.flag & 4 && data.min != 0)
		k = deci_hash_h(data, nbr, i);
	else if (data.min != 0)
		k = deci_min_h(data, nbr, i);
	else
		k = deci_prec_h(data, nbr, i);
	return (k + i);
}
