...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2016/01/28 15:45:46 by rle-mino          #+#    #+#             */
/*   Updated: 2016/02/08 06:43:08 by rle-mino         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "ft_printf.h"

static int		hexa_prec(t_struct data, unsigned int nbr, int i, int lower)
{
	int		k;
...
static int		hexa_min(t_struct data, unsigned int nbr, int i, int lower)
{
	int		j;
	int		k;
...
static int		hexa_hash(t_struct data, unsigned int nbr, int i, int lower)
{
	int		j;
	int		k;
...
int				hexa_conv_noclong(t_struct data, unsigned int nbr, int lower)
{
	...
	k = 0;
	i = pf_rethexa_noclong(nbr, 2);
	if (nbr == 0 && data.min != 0)
		i = 0;
	data.min = data.flag & 0b00000001 ? data.min - 2 : data.min;
	if (data.flag & 0b00000100 && data.min != 0)
		k = hexa_hash(data, nbr, i, lower);
	else if (data.min != 0)
		k = hexa_min(data, nbr, i, lower);
	else
		k = hexa_prec(data, nbr, i, lower);
	if (data.prec != -1)
		return (k + i);
	return (k);
}
